...
The general case is obtained in two steps. First, the absolute case
(when `S` is the constant presheaf of rings on `ULift.{u} ℤ`) reduces
to the case of the identity functor (`absoluteDerivationEquiv`).
Then, given a tower `φ : S ⟶ F.op ⋙ R`, `ψ : R ⟶ G.op ⋙ T` and
`φψ : S ⟶ (F ⋙ G).op ⋙ T` (with `φψ = φ ≫ Functor.whiskerLeft F.op ψ`),
the relative differentials of `ψ` are obtained as the cokernel of a
canonical morphism from the pullback of the relative differentials of `φ`
to the relative differentials of `φψ`
(see `Derivation.Universal.ofIsColimitCokernelCofork` and
`hasDifferentials_of_tower`); applying this to the tower given by
`𝟙 S` and `φ` yields the existence of a universal derivation for any
`φ : S ⟶ F.op ⋙ R` (see the instance `hasDifferentials`), under the
assumption that the pushforward functor of presheaves of modules
induced by `φ` has a left adjoint.

-/

@[expose] public section

-- The proofs in this file rely on defeq unfoldings (e.g. of functor
-- compositions and of coercions of morphisms of presheaves of modules)
-- that require the backward-compatibility flag below.
...
instance : Zero (M.Derivation φ) where
  zero :=
    { d := 0
      d_mul := by simp
      d_map := fun f x ↦ ((M.map f).hom.toAddMonoidHom.map_zero).symm
      d_app := fun _ ↦ rfl }
...
instance : Neg (M.Derivation φ) where
  neg d :=
    { d := -d.d
      ...
      d_map := fun f x ↦ by
        dsimp only [AddMonoidHom.neg_apply]
        rw [d.d_map f x]
        exact ((M.map f).hom.toAddMonoidHom.map_neg (d.d x)).symm
...
instance : Add (M.Derivation φ) where
  add d₁ d₂ :=
    { d := d₁.d + d₂.d
      ...
      d_map := fun f x ↦ by
        dsimp only [AddMonoidHom.add_apply]
        rw [d₁.d_map f x, d₂.d_map f x]
        exact ((M.map f).hom.toAddMonoidHom.map_add (d₁.d x) (d₂.d x)).symm
      d_app := fun _ ↦ by
        dsimp
...
instance : Sub (M.Derivation φ) where
  sub d₁ d₂ :=
    { d := d₁.d - d₂.d
      ...
      d_map := fun f x ↦ by
        dsimp only [AddMonoidHom.sub_apply]
        rw [d₁.d_map f x, d₂.d_map f x]
        exact ((M.map f).hom.toAddMonoidHom.map_sub (d₁.d x) (d₂.d x)).symm
      d_app := fun _ ↦ by
        dsimp
...
instance : AddCommGroup (M.Derivation φ) where
  add_assoc _ _ _ := by ext; simp only [add_d_apply]; rw [add_assoc]
  zero_add _ := by ext; simp only [add_d_apply, zero_d_apply]; rw [zero_add]
  add_zero _ := by ext; simp only [add_d_apply, zero_d_apply]; rw [add_zero]
  neg_add_cancel _ := by
    ext; simp only [add_d_apply, neg_d_apply, zero_d_apply]; rw [neg_add_cancel]
  add_comm _ _ := by ext; simp only [add_d_apply]; rw [add_comm]
  sub_eq_add_neg _ _ := by
    ext; simp only [sub_d_apply, add_d_apply, neg_d_apply]; rw [sub_eq_add_neg]
  nsmul := nsmulRec
  zsmul := zsmulRec
...
/-- When `d : M.Derivation φ` is a universal derivation, this is the bijection
between morphisms `M ⟶ M'` and derivations `M'.Derivation φ`. -/
@[simps]
...
/-- A choice of presheaf of relative differentials of a morphism of presheaves
of commutative rings `φ : S ⟶ F.op ⋙ R` such that `HasDifferentials φ` holds. -/
...
/-- Given a morphism of presheaves of commutative rings `φ : S ⟶ F.op ⋙ R`,
this is the functor which sends a presheaf of `R`-modules `M` to the
abelian group `M.Derivation φ` of relative `φ`-derivations. -/
@[simps obj]
def derivationFunctor :
  ...
  map f := AddCommGrpCat.ofHom
    (AddMonoidHom.mk' (fun d ↦ d.postcomp f) (fun _ _ ↦ by ext; simp))
...
/-- A universal derivation `d : M.Derivation φ` exhibits `M` as a corepresenting
object for the functor sending a presheaf of modules to its set of
...
relative `φ`-derivations by a presheaf of modules `M`. -/
def ofCorepresentableBy : M.Derivation φ := h.homEquiv (𝟙 _)

lemma ofCorepresentableBy_postcomp
    {M' : PresheafOfModules.{v} (R ⋙ forget₂ _ _)} (f : M ⟶ M') :
...
functor of relative `φ`-derivations by a presheaf of modules `M` is universal. -/
...
variable {φ M} {dφ : M.Derivation φ} (hdφ : dφ.Universal)
  ...
  (fac : φψ = φ ≫ Functor.whiskerLeft F.op ψ)
  {P : PresheafOfModules.{v} (T ⋙ forget₂ _ _)}

namespace Derivation

/-- Given `φ : S ⟶ F.op ⋙ R`, `ψ : R ⟶ G.op ⋙ T` and `φψ : S ⟶ (F ⋙ G).op ⋙ T`
such that `φψ = φ ≫ Functor.whiskerLeft F.op ψ`, this is the `φψ`-derivation
induced by a `ψ`-derivation. -/
@[simps]
...
local notation "pushforwardψ" =>
  pushforward (F := G) (R := T ⋙ forget₂ _ _)
    (φ := Functor.whiskerRight ψ (forget₂ _ RingCat))

local notation "pullbackψ" =>
  pullback (F := G) (R := T ⋙ forget₂ _ _)
    (φ := Functor.whiskerRight ψ (forget₂ _ RingCat))

local notation "adjunctionψ" =>
  (pullbackPushforwardAdjunction
    (F := G) (R := T ⋙ forget₂ _ _) (φ := Functor.whiskerRight ψ (forget₂ _ RingCat)))

variable (dφψ : P.Derivation φψ)

/-- Given `φ : S ⟶ F.op ⋙ R`, `ψ : R ⟶ G.op ⋙ T` and `φψ : S ⟶ (F ⋙ G).op ⋙ T`
such that `φψ = φ ≫ Functor.whiskerLeft F.op ψ`, this is the `φ`-derivation
with values in the pushforward of `P` induced by a `φψ`-derivation of `P`. -/
protected noncomputable def pushforward : ((pushforwardψ).obj P).Derivation φ where
  d {X} := AddMonoidHom.mk' (fun a ↦ dφψ.d (ψ.app X a)) (fun a b ↦ by
    have h := map_add dφψ.d (ψ.app X a) (ψ.app X b)
    rw [← map_add (ConcreteCategory.hom (ψ.app X)) a b] at h
    exact h)
  d_mul {X} a b := by
    have h := dφψ.d_mul (X := G.op.obj X) (ψ.app X a) (ψ.app X b)
    rw [← map_mul (ConcreteCategory.hom (ψ.app X)) a b] at h
    exact h
  d_map {X Y} f a := by
    have h : (ψ.app Y) ((R.map f) a) = (T.map (G.op.map f)) ((ψ.app X) a) :=
      congr($(ψ.naturality f) a)
    exact (congr_arg (fun t ↦ dφψ.d t) h).trans (dφψ.d_map (G.op.map f) _)
...
/-- Given a universal `φ`-derivation `dφ : M.Derivation φ` and a `φψ`-derivation
with values in `P`, this is the induced morphism from `M` to the
pushforward of `P`. -/
...
variable [(pushforward (F := G) (R := T ⋙ forget₂ _ _)
  (Functor.whiskerRight ψ (forget₂ _ RingCat))).IsRightAdjoint]

/-- Given a universal `φ`-derivation `dφ : M.Derivation φ` and a `φψ`-derivation
with values in `P`, this is the induced morphism from the pullback of `M` to `P`. -/
...
/-- Given a universal `φ`-derivation `dφ : M.Derivation φ`, a universal
`φψ`-derivation `dφψ : P.Derivation φψ`, and a colimit cokernel cofork `c` of
the canonical morphism from the pullback of `M` to `P`, the point of `c`
corepresents the functor of relative `ψ`-derivations. -/
...
/-- Given a universal `φ`-derivation, a universal `φψ`-derivation with values in `P`,
and a colimit cokernel cofork `c` of the canonical morphism from the pullback of `M`
to `P`, the point of `c` identifies to the presheaf of relative differentials of `ψ`:
the corresponding `ψ`-derivation is universal. -/
...
/-- Derivations relative to a morphism `φ` from the constant presheaf
of commutative rings on `ULift.{u} ℤ` (over any functor `F : C ⥤ D`) identify
to derivations relative to the canonical morphism from this constant presheaf
over the identity functor of `D`. -/
def absoluteDerivationEquiv
    ...
    M.Derivation φ ≃ M.Derivation (F := 𝟭 D)
      ...
      { app := fun _ ↦ CommRingCat.isInitial.{u}.to _
        naturality := fun _ _ _ ↦ CommRingCat.isInitial.hom_ext _ _ } where
  toFun d :=
    { d := d.d
      d_mul := by simp
      d_map := by simp
      d_app := fun a ↦ d.d_ulift_int_eq_zero _ _ _ }
  invFun d :=
    { d := d.d
      d_mul := by simp
      d_map := by simp
...
/-- The equivalence of universal properties deduced from `absoluteDerivationEquiv`. -/
def absoluteDerivationUniversalEquiv
    ...
    (d : M.Derivation (F := 𝟭 D)
      (S := (Functor.const Dᵒᵖ).obj (CommRingCat.of (ULift.{u} ℤ))) (R := R)
      { app := fun _ ↦ CommRingCat.isInitial.{u}.to _
        naturality := fun _ _ _ ↦ CommRingCat.isInitial.hom_ext _ _ }) :
...
instance hasDifferentials
    [(pushforward.{u} (F := F) (R := R ⋙ forget₂ _ _)
      (Functor.whiskerRight φ (forget₂ _ RingCat))).IsRightAdjoint] : HasDifferentials φ := by
